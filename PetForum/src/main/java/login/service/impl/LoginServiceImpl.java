...
public class LoginServiceImpl implements LoginService {
	...
	public Map<String, Object> updateUserInApp(UserInfo userInfo, String token) {
		Map<String, Object> respMap = new HashMap<String, Object>();
		userInfo.setToken(token);
		try{
			loginMapper.updatePic(userInfo);
	...
	public Map<String, Object> updatePassWordInApp(UserInfo userInfo,
			String token) {
		Map<String, Object> respMap = new HashMap<String, Object>();
		userInfo.setToken(token);
		try{
			MailModel mailModel = loginMapper.getYzmInValue(userInfo);
			if(mailModel==null){
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
				respMap.put(ConstDefine.CONST_MESSAGE_KEY, "验证码不正确");
			}else{
				Date date = commonMapper.getDbDate();
				long [] times= CommonFun.getDistanceTimes(mailModel.getCreate_time(),date);
				if(times[0]>0 || times[1]>0 || times[2]>5){
					respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
					respMap.put(ConstDefine.CONST_MESSAGE_KEY, "验证码超时，请重新发送");
				}else{
					loginMapper.updatePassWord(userInfo);
					respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
				}
	...
	public Map<String, Object> getImgInUser() {
		Map<String, Object> respMap = new HashMap<String, Object>();
		List<String> list = new ArrayList<String>();
		try{
			List<String> headList = loginMapper.getImgInUser();
			Collections.shuffle(headList);
			int toIndex = headList.size()>4?5:headList.size();
			list = headList.subList(0,toIndex);
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			respMap.put(ConstDefine.CONST_DATA_KEY, list);
		}catch(Exception e){
			logger.error(e.getMessage());
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
			respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.SERVELT_ERROR);
