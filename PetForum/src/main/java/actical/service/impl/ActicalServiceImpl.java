...
public class ActicalServiceImpl implements ActicalService {
	...
	public Map<String, Object> getActList(int pageIndex,int pageSize,SearchModel searchModel) {
		...
		paramsMap.put("tag", searchModel.getTag());
		Map<String,Object> respMap = new HashMap<String,Object>();
	...
	public Map<String, Object> delAct(int id,String token) {
		Map<String,Object> respMap = new HashMap<String,Object>();
		try{
			int count = mapper.isTrueSend(token,id);
			if(count == 0){
				//不是本人发布
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
				respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.OTHERDEL_ERROR);
			}else{
				mapper.delAct(id);
				mapper.delReplay(id);
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
	...
	public Map<String, Object> delReplay(int replayId,String token) {
		Map<String,Object> respMap = new HashMap<String,Object>();
		try{
			int count = mapper.isTrueReplay(token,replayId);
			if(count==0){
				//不是本人发布
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
				respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.OTHERDEL_ERROR);
	...
	public Map<String, Object> findHasById(int id) {
		Map<String,Object> respMap = new HashMap<String,Object>();
		try{
			int count  = mapper.findHasById(id);
			if(count==0){
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
				respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.NOTACT_ERROR);
			}else{
				respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			}
		}catch(Exception e){
	...
	public Map<String, Object> getInfoNotRead(String token) {
		Map<String,Object> respMap = new HashMap<String,Object>();
		try{
			int count  = mapper.getInfoNotRead(token);
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			respMap.put(ConstDefine.CONST_DATA_KEY, count);
		}catch(Exception e){
			logger.error(e.getMessage());
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
			respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.SERVELT_ERROR);
	...
	public Map<String, Object> getTag() {
		...
		try{
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			respMap.put(ConstDefine.CONST_DATA_KEY, mapper.getTag());
		}catch(Exception e){
			logger.error(e.getMessage());
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
			respMap.put(ConstDefine.CONST_MESSAGE_KEY, BusinessError.SERVELT_ERROR);
	...
	public Map<String, Object> inKeep(KeepModel keepModel,String token) {
		Map<String,Object> respMap = new HashMap<String,Object>();
		try{
			int userId = loginMapper.getUidFromToken(token);
			keepModel.setUserId(userId);
			mapper.inKeep(keepModel);
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
		}catch(Exception e){
			logger.error(e.getMessage());
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, false);
	...
	public Map<String, Object> isKeep(KeepModel keepModel, String token) {
		...
		try{
			int userId = loginMapper.getUidFromToken(token);
			keepModel.setUserId(userId);
			int count = mapper.isKeep(keepModel);
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			respMap.put(ConstDefine.CONST_DATA_KEY, count);
		}catch(Exception e){
			logger.error(e.getMessage());
	...
	public Map<String, Object> getMyKeepAct(int pageIndex, int pageSize,
			String userId) {
		Map<String,Object> paramsMap = new HashMap<String,Object>();
		paramsMap.put("userId", userId);
		Map<String,Object> respMap = new HashMap<String,Object>();
		...
		paramsMap.put("pageSize", pageSize);
		try{
			List<ActicalModel> dataList = mapper.getMyKeepAct(paramsMap);
			int count  = mapper.getKeepCount(paramsMap);
			int pageCount = (count/pageSize)+(count%pageSize==0?0:1);
			Pagination<ActicalModel> pagination = new Pagination<ActicalModel>(pageIndex, pageSize, pageCount, count, dataList);
			respMap.put(ConstDefine.CONST_SUCCESS_KEY, true);
			respMap.put(ConstDefine.CONST_DATA_KEY, pagination);
